...
sudo docker cp /volume1/docker/cleanvid2/src/cleanvid/. cleanvid2:/app/src/cleanvid/
# Restart
echo -e "${CYAN}==>${NC} Restarting container..."
sudo docker-compose restart
sleep 5
if sudo docker ps | grep -q cleanvid2; then
    ...
    sudo docker logs cleanvid2 --tail 10
    echo ""
    echo "🌐 Dashboard: http://$(hostname -I | awk '{print $1}'):8080"
    echo "💡 To view live logs: sudo docker logs -f cleanvid2"
else
