...
class Processor:
    ...
    def process_batch(
        ...
        for i, video_path in enumerate(videos, 1):
            # Check time limit
            if max_time_minutes:
                elapsed_minutes = (datetime.now() - stats.start_time).total_seconds() / 60
                if elapsed_minutes >= max_time_minutes:
                    print(f"\n⏱️  Time limit reached ({elapsed_minutes:.1f}/{max_time_minutes} minutes)")
                    print(f"Stopping batch processing. Processed {i-1}/{len(videos)} videos.")
                    break
    ...
    def print_status(self) -> None:
        ...
        print(f"  OpenSubtitles: {'Enabled' if status['opensubtitles_enabled'] else 'Disabled'}")
